...
public class Te : MonoBehaviour
{
	...
	public void stopLine()
	{
		...
		StartCoroutine (addVertexes());
	}

	IEnumerator addVertexes()
	{
		var tempPointsList = pointsList.ToList ();
		Vector3 deltaLine = pointsList[pointsList.Count - 1] - pointsList [0];

		Debug.LogErrorFormat ("tempPointsList count: {0}. pointsListCount: {1}", tempPointsList.Count, pointsList.Count);
		...
		for (int i = 0; i < tempPointsList.Count; i++)
		{
			...
			addVertex (tempPointsList[i], true);
		}
	}
